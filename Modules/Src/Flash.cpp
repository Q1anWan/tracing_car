...
bool cFlashCore::init() {
    ...
    for (auto &data: _flash_list) {
        ...
        // Check head
        if (flash_struct.crc[1] == CRC8Maxim(address, sizeof(Flash_t) - FLASH_STORAGE_CRC_FILED_SIZE)) {
...
bool cFlashCore::config_data(Flash_Element_ID element_id, uint8_t *data, uint32_t len) {
    ...
    _flash_list[element_id].crc[0] = CRC8Maxim(data, len);
...
bool cFlashCore::rebuild() {
    ...
    for (auto &data: _flash_list) {
        ...
        data.crc[1] = CRC8Maxim((uint8_t *) &data, sizeof(Flash_t) - FLASH_STORAGE_CRC_FILED_SIZE);
...
bool cFlashCore::flash_memcpy(Flash_Element_ID element_id, uint8_t *dst, bool en_crc) {
    ...
    if (en_crc) {
        if (CRC8Maxim(dst, _flash_list[element_id].bytes) != _flash_list[element_id].crc[0]) {
            return false;
        }
    }
